...
# Sidebar element for year selection
...
# Sidebar element for Team selection
...
# Sidebar element for position selection

unique_pos = ['C', 'PF', 'SF', 'PG', 'SG']
selected_pos = st.sidebar.multiselect('Position', unique_pos, unique_pos)

# Filtering data
selected_team_bool = ((playerstats.Tm.isin(
    selected_team)) & (playerstats.Pos.isin(selected_pos)))
df_selected_team = playerstats[selected_team_bool]

st.header('Display Player Stats of Selected Team(s)')
st.write('Data Dimension: ' + str(df_selected_team.shape[0]) + ' rows and ' + str(
    df_selected_team.shape[1]) + ' columns.')
# have to use astype(str) because of a bug with streamlit, original code does not have this
